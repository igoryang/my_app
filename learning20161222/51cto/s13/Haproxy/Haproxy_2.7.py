...
def name_decide():
    ...
    while name_flag:
        name_input = raw_input('33[32m请输入新服务名:(退出输入q)33[0m')
        if len(name_input) == 0:  ###如果输入为空则直接下一次循环###
            continue
        elif name_input == 'q':  ###输入q，退出本次输入###
            name_flag = False
        elif re.match('[0-9a-zA-Z_]+', name_input):  ###匹配输入是否以字符、数字或下划线开头###
            name = name_input
            name_flag = False  ###输入成功后退出循环###
        else:
            print('33[31m名称输入错误，请重新输入!!!33[0m')
    return (name)  ###返回输入的结果###
...
def ipaddress_decide():
    ...
    while address_flag:
        address_input = raw_input('33[32m请输入新地址(IP哦):(退出输入q)33[0m')
        ...
        ##################匹配输入是否是ip:port的格式######################
            ...
            print('33[31m地址输入错误，请重新输入!!!33[0m')
...
def number_decide(name):
    ...
    while number_flag:
        number_input = raw_input('33[32m请输入 %s (数字哦):(退出输入q)33[0m' % name)
        if len(number_input) == 0:  ###如果输入为空则直接下一次循环###
            ...
            try:
                ...
                print('33[31m %s 输入错误，请重新输入!!!33[0m' % name)
            else:
                number = number_input
...
if __name__ == '__main__':
    ...
    while flag:
        ...
        print('
              == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == ')
        print('33[33m 欢迎访问haproxy配置文件管理平台：33[0m')
        print('
        backend列表信息如下: ')
        ...
        print
        '''
        33[31m 1. 查询后端服务33[0m
        33[32m 2. 添加后端服务33[0m
        33[33m 3. 修改后端服务33[0m
        33[35m 4. 删除后端服务33[0m
        33[36m 5. 退出33[0m '''
        print('
              == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == ')

        select_num = raw_input('33[33m请选择操作条目：33[0m')

        #######################1. 查询后端服务################################
        if select_num == '1':
            query_flag = True  ###初始化查询循环标志###
        while query_flag:
            backend_index = raw_input('33[32;1m请输入查询的backend编号或名称(返回上层菜单,请输入b)：33[0m')
        ...
        if select_num == '2':
            ...
            while add_flag:
                backend_index = raw_input('33[32;1m请输入添加的backend的编号或名称(返回上层菜单,请输入b)：33[0m')
                ##############################对输入值进行判断################
                ...
                if backend_name:  ###显示对应backend下的server列表###
                    ...
                    print(add_server_dict['name'], add_server_dict['address'], add_server_dict['weight'],
                          add_server_dict['maxconn'])

                    #############对输入的四个服务信息是否成功判断#############
                    if add_server_dict['name'] and add_server_dict['address'] and add_server_dict['weight'] and \
                            add_server_dict['maxconn']:
        ...
        if select_num == '3':
            ...
            while backend_modify_flag:
                backend_index = raw_input('33[32;1m请输入修改的backend的编号或名称(返回上层菜单,请输入b)：33[0m')
                ##############################对输入值进行判断################
                ...
                if backend_name:  ###显示对应backend下的server列表###
                    ...
                    while server_modify_flag:
                        ...
                        server_index = raw_input('33[32;1m请输入修改的server的编号(返回上层菜单,请输入b)：33[0m')
                        ...
                        #####################指定具体的server编号进行判断#####
                            ...
                            print(modify_server_dict['name'], modify_server_dict['address'],
                                  modify_server_dict['weight'], modify_server_dict['maxconn'])

                            ######对输入的四个服务信息是否成功判断#############
                            if modify_server_dict['name'] and modify_server_dict['address'] and modify_server_dict[
                                'weight'] and modify_server_dict['maxconn']:
